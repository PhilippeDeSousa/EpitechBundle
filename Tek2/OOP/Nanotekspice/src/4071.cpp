...
nts::Chip4071::Chip4071(std::string name)
        : Name(name)
{
        for (int i = 0; i < 15; i++) {
...
void nts::Chip4071::dump() const
{
        std::cout << Name + ": Chip 4071\n";
        for (int i = 1; i < 15; i++) {
                std::cout << "\tPin n°";
                std::cout << i;
                std::cout << ", value: ";
...
std::vector<nts::Tristate> &nts::Chip4071::getPin()
{
        return chipPins;
}

nts::Tristate nts::Chip4071::getValue(nts::Tristate first,
				      nts::Tristate second)
{
...
void nts::Chip4071::setLink(std::size_t pin, nts::IComponent &other, std::
        size_t otherPin)
{
        if (pin < 1 || pin > 14)
                throw (PinException("PinException: pin "
...
std::string nts::Chip4071::getName() const
{
        return Name;
}

nts::Chip4071::~Chip4071()
{}
