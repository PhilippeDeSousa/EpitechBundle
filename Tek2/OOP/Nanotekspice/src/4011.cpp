...
nts::Chip4011::Chip4011(std::string name)
        : Name(name)
{
        for (int i = 0; i < 15; i++) {
...
void nts::Chip4011::dump() const
{
        std::cout << Name + ": Chip 4011\n";
        for (int i = 1; i < 15; i++) {
                std::cout << "\tPin n°";
                std::cout << i;
                std::cout << ", value: ";
...
std::vector<nts::Tristate> &nts::Chip4011::getPin()
{
        return chipPins;
}

nts::Tristate nts::Chip4011::getValue(nts::Tristate first,
				      nts::Tristate second)
{
...
void nts::Chip4011::setLink(std::size_t pin, nts::IComponent &other, std::
        size_t otherPin)
{
        if (pin < 1 || pin > 14)
                throw (PinException("PinException: pin "
...
std::string nts::Chip4011::getName() const
{
        return Name;
}

nts::Chip4011::~Chip4011()
{}
