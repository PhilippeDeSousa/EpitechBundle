...
nts::Chip4040::Chip4040(std::string name)
        : Name(name)
{
        for (int i = 0; i < 14; i++) {
...
void nts::Chip4040::dump() const
{
        std::cout << Name + ": Chip 4040\n";
        for (int i = 1; i < 15; i++) {
                std::cout << "\tPin n°";
                std::cout << i;
                std::cout << ", value: ";
...
std::vector<nts::Tristate> &nts::Chip4040::getPin()
{
        return chipPins;
}

nts::Tristate nts::Chip4040::getValue(nts::Tristate first,
				      nts::Tristate second)
{
...
nts::Tristate nts::Chip4040::compute(std::size_t pin)
{
        if (pin < 1)
...
void nts::Chip4040::setLink(std::size_t pin, nts::IComponent &other, std::
        size_t otherPin)
{
        if (pin < 1 || pin > 14)
                throw (PinException("PinException: pin "
...
std::string nts::Chip4040::getName() const
{
        return Name;
}

nts::Chip4040::~Chip4040()
{}
